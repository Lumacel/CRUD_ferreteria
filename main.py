...
class App():
    """ Esta clase controla la interface grafica"""
    ...
    def cargar_distribuidoras(self):
        ...
        try:
            ...
            messagebox.showinfo(
                message="ERROR!!! NO SE PUDIERON CARGAR LAS DISTRIBUIDORAS", title="INFO")
    ...
    def ventana_principal(self):
        ...
        self.menu_archivo = tk.Menu(
            self.root, tearoff=False, font=("Arial", 11))
        self.menu_archivo.add_command(
            label="Explorar archivos", accelerator="F3", command=self.ver_archivos_normalizados)
        self.menu_archivo.add_command(
            label="Normalizar listas", accelerator="F4", command=self.toplevel_normalizar)
        self.menu_archivo.add_separator()
        self.menu_archivo.add_command(label="Salir", command=self.root.quit)

        # Crear el menú Editar
        self.menu_ventas = tk.Menu(
            self.root, tearoff=False, font=("Arial", 11))
        self.menu_ventas.add_command(label="Efectuar venta", accelerator="F2", command=lambda: self.aceptar_venta(
        ) if len(self.ventas.items_vta) != 0 else None)
        self.menu_ventas.add_command(
            label="Aplicar descuento global", accelerator="F5", command=self.toplevel_dto_global)
        self.menu_ventas.add_command(
            label="Agregar artículo", accelerator="F10", command=lambda: self.editar_item_vta("AGREGAR"))
        self.menu_ventas.add_command(label="Editar artículo", accelerator="Enter", command=lambda: self.editar_item_vta(
            "EDITAR") if self.tabla_venta.selection() != () else None)
        self.menu_ventas.add_command(
            label="Borrar artículo/s", accelerator="Ctrl + Del", command=self.borrar_items_vta)
        self.menu_ventas.add_command(
            label="Modificar medio de pago", accelerator="F8", command=self.modif_medio_pago)
        self.menu_ventas.add_command(
            label="Reiniciar venta", accelerator="Ctrl + R", command=self.resetear_venta)

        # Crear el menú Configuración
        self.menu_config = tk.Menu(
            self.root, tearoff=False, font=("Arial", 11))
        self.menu_config.add_command(
            label="Modificar margen ganancia", accelerator="F6", command=self.toplevel_ganancia)
        self.menu_config.add_command(
            label="Registros historicos", accelerator="F7", command=self.toplevel_registros)

        # Crear la barra de menús y agregar los menús
        self.barra_menus = tk.Menu(self.root)

        self.barra_menus.add_cascade(
            label="   Archivos", menu=self.menu_archivo)
        self.barra_menus.add_cascade(label=" Ventas", menu=self.menu_ventas)
        self.barra_menus.add_cascade(
            label=" Configuración", menu=self.menu_config)
        ...
        self.lbl_buscar = tk.Label(self.root, text="  BUSCAR ARTICULO  ",
                                   relief="ridge", width=20, bg="lightblue", fg="black", font=("Arial", 11))
        self.lbl_buscar.place(x=12, y=10)
        self.entry_buscar = tk.Entry(
            self.root, textvariable=self.articulo_busq, width=74, font=("Arial", 11))
        self.entry_buscar.place(x=200, y=10)
        self.lbl_distr = tk.Label(self.root, text="DISTRIBUIDORA", width=20,
                                  relief="ridge", bg="lightblue", fg="black", font=("Arial", 11))
        self.lbl_distr.place(x=814, y=10)
        self.entry_distr = ttk.Combobox(self.root, textvariable=self.distr_selecc, values=self.get_distribuidoras(
        ), state="readonly", width=20, height=len(self.get_distribuidoras()))
        ...
        self.lbl_modo_pago = tk.Label(self.root, text="-- EFECTIVO --", relief="sunken",
                                      width=24, bg="lightblue", fg="blue", justify="right", font=("Arial", 11))
        self.lbl_modo_pago.place(x=25, y=260)
        self.lbl_subtotal_name = tk.Label(
            self.root, text="SUBTOTAL ", relief="ridge", width=12, bg="lightblue", fg="black", font=("Arial", 11))
        self.lbl_subtotal_name.place(x=328, y=260)
        self.lbl_subtotal_valor = tk.Label(self.root, textvariable=self.subtotal, relief="sunken",
                                           width=12, bg="white", fg="black", justify="right", font=("Arial", 11))
        self.lbl_subtotal_valor.place(x=443, y=260)
        self.lbl_descuento_name = tk.Label(
            self.root, text="DESCUENTO ", relief="ridge", width=12, bg="lightblue", fg="black", font=("Arial", 11))
        self.lbl_descuento_name.place(x=627, y=260)
        self.lbl_descuento_valor = tk.Label(self.root, textvariable=self.descuento, relief="sunken",
                                            width=12, bg="white", fg="black", justify="right", font=("Arial", 11))
        self.lbl_descuento_valor.place(x=743, y=260)
        self.lbl_total_name = tk.Label(
            self.root, text="A PAGAR ", relief="ridge", width=12, bg="lightblue", fg="black", font=("Arial", 11))
        ...
        self.root.bind("<Double-Button-1>", lambda x: self.editar_item_vta("EDITAR")
                       if self.tabla_venta.selection() != () else self.entry_buscar.focus())
        self.root.bind("<Control-r>", lambda x: self.resetear_venta())
        self.root.bind("<Escape>", lambda x:  self.escape_root())
        self.root.bind("<F2>", lambda x: self.aceptar_venta()
                       if len(self.ventas.items_vta) != 0 else None)
    ...
    def toplevel_ganancia(self):
        ...
        self.lbl_ganancia = tk.Label(self.toplevel_gan, text="GANANCIA % ",
                                     bg="lightblue", relief="ridge", width=13, font=("Arial", 11))
        self.lbl_ganancia.place(x=12, y=20)
        self.entry_ganancia = tk.Entry(
            self.toplevel_gan, textvariable=self.porcentaje_ganancia, width=15, justify="right", font=("Arial", 11))
        self.entry_ganancia.place(x=138, y=20)
        self.lbl_clave = tk.Label(self.toplevel_gan, text="CONTRASEÑA",
                                  bg="lightblue", relief="ridge", width=13, font=("Arial", 11))
        ...
        self.btn_ganancia_aceptar = tk.Button(
            self.toplevel_gan, text=" ACEPTAR ", width=10, command=lambda: self.cambiar_valor_ganancia(), font=("Arial", 11))
        self.btn_ganancia_aceptar.place(x=12, y=100)
        self.btn_ganancia_salir = tk.Button(
            self.toplevel_gan, text=" SALIR ", width=10, command=lambda: self.toplevel_gan.destroy(), font=("Arial", 11))
        ...
        self.toplevel_gan.bind(
            "<Return>", lambda x: self.cambiar_valor_ganancia())
        self.toplevel_gan.bind(
            "<Escape>", lambda x: self.toplevel_gan.destroy())
    ...
    def cambiar_valor_ganancia(self):
        ...
        try:
            ...
            if 0 <= porc_gan <= 100 and self.password.get() == "faloelportugues":
                ...
                messagebox.showinfo(
                    message=f"  EL PORCENTAJE DE GANANCIA AHORA ES {porc_gan}%  ", title="INFO")
    ...
    def modif_medio_pago(self):
        if messagebox.askyesno(title="MEDIO DE PAGO", message="PAGO CON TARJETA" if self.ventas.medio_pago else "PAGO EN EFECTIVO"):
            ...
            self.lbl_modo_pago.config(
                text="-- EFECTIVO --" if self.ventas.medio_pago else "-- TARJETA --")
        else:
            None

    def return_root(self):
        # al presionar <enter> realiza búsqueda si el foco está en el entry de busqueda
        if isinstance(self.root.focus_get(), tk.Entry):
            self.buscar_item()
        else:
            self.editar_item_vta(
                "EDITAR") if self.tabla_venta.selection() != () else None
    ...
    def toplevel_dto_global(self):
        ...
        self.lbl_dto = tk.Label(self.toplevel_dto, text="DESCUENTO %",
                                bg="lightblue", relief="ridge", width=13, font=("Arial", 11))
        self.lbl_dto.place(x=12, y=20)
        self.entry_dto = tk.Entry(
            self.toplevel_dto, textvariable=self.dto_global, width=15, justify="right", font=("Arial", 11))
        self.entry_dto.place(x=138, y=20)
        self.btn_dto_aceptar = tk.Button(self.toplevel_dto, text=" ACEPTAR ",
                                         width=10, command=lambda: self.aplicar_dto_global(), font=("Arial", 11))
        self.btn_dto_aceptar.place(x=12, y=60)
        self.btn_dto_salir = tk.Button(self.toplevel_dto, text=" SALIR ", width=10,
                                       command=lambda: self.toplevel_dto.destroy(), font=("Arial", 11))
        ...
        self.toplevel_dto.bind(
            "<Escape>", lambda x: self.toplevel_dto.destroy())
    ...
    def aplicar_dto_global(self):
        try:
            ...
            if descuento >= 0 and descuento <= 100:
                ...
                messagebox.showinfo(
                    message="SOLO SE PERMITEN VALORES NUMERICOS (0-100)", title="INFO")
                self.dto_global.set(self.get_dto_global())
                self.entry_dto.focus()
                self.entry_dto.select_range(0, 'end')
                self.entry_dto.icursor('end')
    ...
    def toplevel_resultados(self):  # --- configura ventana para entrada de datos
        ...
        self.lbl_distr_resultados = tk.Label(
            self.toplevel_result, text="DISTRIBUIDORA", width=20, relief="ridge", bg="lightblue", fg="black", font=("Arial", 11))
        self.lbl_distr_resultados.place(x=745, y=210)
        self.lbl_item_resultados = tk.Label(
            self.toplevel_result, text="BUSQUEDA", relief="ridge", width=20, bg="lightblue", fg="black", font=("Arial", 11))
        self.lbl_item_resultados.place(x=12, y=210)
        self.entry_item_resultados = tk.Label(
            self.toplevel_result, text=(self.articulo_busq.get()).upper(), relief="sunken", width=20, font=("Arial", 11))
        self.entry_item_resultados.place(x=200, y=210)
        self.entry_distr_resultados = ttk.Combobox(
            self.toplevel_result, textvariable=self.distr_selecc, values=self.get_distribuidoras(), state="readonly", width=20, height=len(self.get_distribuidoras()))
        ...
        self.toplevel_result.bind(
            "<<ComboboxSelected>>", lambda x: self.combobox_select())
        # destruye ventana al apretar Escape
        self.toplevel_result.bind(
            "<Escape>", lambda x: self.esc_toplevel_resultados())
        self.toplevel_result.bind(
            "<Return>", lambda x: self.agregar_artic_venta())  # carga articulo
        self.toplevel_result.bind(
            "<Double-Button-1>", lambda x: self.agregar_artic_venta())
        self.toplevel_result.protocol(
            "WM_DELETE_WINDOW", self.esc_toplevel_resultados)
    ...
    def treeview_busqueda(self, filas=8):  # --- da formato a la tabla treeview
        columnas = ("CODIGO", "ARTICULO", "P. COMPRA $",
                    "P. VENTA $", "DISTRIBUIDORA")
        self.tabla_resultados = ttk.Treeview(
            self.toplevel_result, height=filas, show="headings", columns=(columnas))
        self.tabla_resultados.place(x=10, y=10)

        # --- barra scroll
        self.scroll = tk.Scrollbar(
            self.toplevel_result, orient="vertical", command=self.tabla_resultados.yview)
        ...
        self.tabla_resultados.column(
            "CODIGO", anchor=tk.W, width=100, minwidth=120)
        self.tabla_resultados.column(
            "ARTICULO", anchor=tk.W, width=640, minwidth=620)
        self.tabla_resultados.column(
            "P. COMPRA $", anchor=tk.E, width=100, minwidth=100)
        self.tabla_resultados.column(
            "P. VENTA $", anchor=tk.E, width=100, minwidth=100)
        self.tabla_resultados.column(
            "DISTRIBUIDORA", anchor=tk.CENTER, width=155, minwidth=155)
        ...
        self.tabla_resultados.heading(
            "#3", text="P. COMPRA $", anchor=tk.CENTER)
        self.tabla_resultados.heading(
            "#4", text="P. VENTA $", anchor=tk.CENTER)
        self.tabla_resultados.heading(
            "#5", text="DISTRIBUIDORA", anchor=tk.CENTER)

    def treeview_venta(self, filas=8):
        columnas = ("ARTICULO", "CANT.", "PRECIO",
                    "SUBTOTAL", "DTO. %", "DTO. $", "TOTAL")
        self.tabla_venta = ttk.Treeview(
            self.root, height=filas, show="headings", columns=(columnas))
        ...
        self.scroll_venta = tk.Scrollbar(
            self.root, orient="vertical", command=self.tabla_venta.yview)
        ...
        self.tabla_venta.column("ARTICULO", anchor=tk.W,
                                width=570, minwidth=570)
        self.tabla_venta.column(
            "CANT.", anchor=tk.CENTER, width=50, minwidth=50)
        self.tabla_venta.column("PRECIO", anchor=tk.E, width=100, minwidth=100)
        self.tabla_venta.column("SUBTOTAL", anchor=tk.E,
    ...
    # --- agrega datos a la tabla (treeview)
    def cargar_treeview_resultados(self):
        for item in self.lista_coincidencias:
            if item[3] == self.get_distr_selecc() or self.get_distr_selecc() == "TODAS":
                ...
                lista = (codigo, articulo, precio_compra,
                         precio_venta, distribuidora)
                self.tabla_resultados.insert(
                    "", tk.END, text="", values=(lista))
            else:
                continue

    def agregar_artic_venta(self):
        # items seleccionados (marcados en azul)
        items_selecc = self.tabla_resultados.selection()
        if len(items_selecc) > 0:
            for i in items_selecc:
                item = self.tabla_resultados.item(i)["values"]

                v = Ventas(item[0], item[1], 1, self.get_dto_global(
                ), item[3], item[4])  # crea objeto Ventas
    ...
    def cargar_treeview_venta(self):
        ...
        for item in self.ventas.items_vta:
            ...
            lista = (articulo, cantidad, f"{precio:.2f}", f"{subtotal:.2f}",
                     f"{descuento:.2f} %", f"{total_desc:.2f}", f"{total:.2f}")
    ...
    def borrar_items_vta(self):
        # items seleccionados (marcados en azul)
        items_selecc = self.tabla_venta.selection()
        # Devuelve tupla con los id de elementos
    ...
    def toplevel_editar(self):
        ...
        self.lbl_cant_editar = tk.Label(
            self.ventana_editar, text="CANT.", relief="ridge", width=5, bg="lightblue", font=("Arial", 11))
        self.lbl_cant_editar.place(x=1, y=5)
        self.lbl_artic_editar = tk.Label(
            self.ventana_editar, text="ARTICULO", relief="ridge", width=58, bg="lightblue", font=("Arial", 11))
        self.lbl_artic_editar.place(x=52, y=5)
        self.lbl_artic_precio = tk.Label(
            self.ventana_editar, text="PRECIO $", relief="ridge", width=12, bg="lightblue", font=("Arial", 11))
        self.lbl_artic_precio.place(x=580, y=5)
        self.lbl_artic_descuento = tk.Label(
            self.ventana_editar, text="DTO. %", relief="ridge", width=8, bg="lightblue", font=("Arial", 11))
        self.lbl_artic_descuento.place(x=694, y=5)
        self.entry_cant_editar = tk.Entry(
            self.ventana_editar, textvariable=self.cant_editar, width=6, bg="white", justify="center", font=("Arial", 11))
        self.entry_cant_editar.place(x=1, y=28)
        self.entry_artic_editar = tk.Entry(
            self.ventana_editar, textvariable=self.artic_editar,  width=66, bg="white", justify="left", font=("Arial", 11))
        self.entry_artic_editar.place(x=52, y=28)
        self.entry_artic_precio = tk.Entry(
            self.ventana_editar, textvariable=self.precio_editar, width=14, bg="white", justify="right", font=("Arial", 11))
        self.entry_artic_precio.place(x=580, y=28)
        self.entry_artic_descuento = tk.Entry(
            self.ventana_editar, textvariable=self.dto_editar, width=9, bg="white", justify="right", font=("Arial", 11))
        self.entry_artic_descuento.place(x=694, y=28)
        self.lbl_fraccionar = tk.Label(
            self.ventana_editar, text="FRACCIONAR", relief="ridge", width=12, state="disabled", bg="lightblue", font=("Arial", 10))
        self.lbl_fraccionar.place(x=5, y=70)
        self.entry_fraccionar = tk.Entry(
            self.ventana_editar, textvariable=self.fracc_editar, width=5, state="disabled", bg="white", justify="right", font=("Arial", 11))
        self.entry_fraccionar.place(x=110, y=70)
        self.check_btn_fracc = ttk.Checkbutton(
            self.ventana_editar, text="Habilitar", variable=self.fracc_state, command=self.fracc_seleccion)
        ...
        if self.modo == "EDITAR":
            ...
            # en modo agregar agrega a la lista un item con valores nulos
        ...
        # simula un click para activar ventana y cursor
        self.ventana_editar.event_generate("<Button-1>")

        # llama a funcion escape_editar (destruye ventana al apretar Escape)
        self.ventana_editar.bind("<Escape>", lambda x: self.escape_editar())
        self.ventana_editar.bind(
            "<Return>", lambda x: self.validar_entradas(item))  # carga articulo
        self.ventana_editar.bind(
            "<KeyRelease>", lambda x: self.key_released_fracc())
        # llama a la funcion escape_editar cuando se cierra la ventana
    ...
    # verifica si se fracciona el producto cuando se ingresa un valor
    ...
    def validar_entradas(self, item):
        try:
            ...
            precio = round((float(self.precio_original if self.fracc_state.get(
            ) else self.precio_editar.get())), 2)
            ...
            if cantidad == 0 or descuento < 0 or descuento > 100 or precio <= 0 or articulo == "":
                messagebox.showinfo(
                    message="VERIFIQUE QUE LOS VALORES INGRESADOS SEAN CORRECTOS", title="INFO")
                self.reset_campo_cantidad()

            else:
                self.ventas.editar_articulo(
                    articulo, cantidad, precio, fraccion, descuento, item)
    ...
    # posiciona cursor en el campo y selecciona valores
    ...
    def buscar_item(self):
        ...
        if len(articulo) > 2:
            ...
            if len(self.lista_coincidencias) == 0:
                messagebox.showinfo(
                    message="NO HAY COINCIDENCIAS PARA SU BÚSQUEDA", title="INFO")
                self.entry_buscar.select_range(0, 'end')
                self.entry_buscar.icursor('end')
            else:
            ...
            messagebox.showinfo(
                message="LA BÚSQUEDA DEBE CONTENER AL MENOS 3 CARACTERES", title="INFO")
    ...
    def toplevel_normalizar(self):
        ...
        self.lbl_modelo = tk.Label(
            self.ventana_normalizar, text=" SELECCIONAR MODELO ", bg="lightblue", relief="ridge", width=22, font=("Arial", 11))
        self.lbl_modelo.place(x=10, y=20)
        self.entry_modelo = ttk.Combobox(
            self.ventana_normalizar, textvariable=self.modelo_distr_norm, values=lista_distr, state="readonly", width=18, height=len(lista_distr))
        self.entry_modelo.place(x=220, y=20)
        self.entry_modelo.config(font=("Arial", 11))
        self.btn_aceptar = tk.Button(
            self.ventana_normalizar, text=" ARCHIVO ", width=10, command=self.normalizar_lista, font=("Arial", 11))
        self.btn_aceptar.place(x=10, y=60)
        self.btn_salir = tk.Button(
            self.ventana_normalizar, text=" SALIR ", width=10, command=lambda: self.ventana_normalizar.destroy(), font=("Arial", 11))
        self.btn_salir.place(x=288, y=60)

        self.btn_aceptar.bind("<Return>", lambda x: self.normalizar_lista())
        self.btn_salir.bind(
            "<Return>", lambda x: self.ventana_normalizar.destroy())
        self.ventana_normalizar.bind(
            "<Escape>", lambda x: self.ventana_normalizar.destroy())
        self.entry_modelo.bind("<<ComboboxSelected>>",
                               lambda x: self.btn_aceptar.focus())
    ...
    def toplevel_pago_tarjeta(self):
        ...
        self.monto_tarjeta.set(total) if len(
            self.ventas.items_vta) != 0 else None
        ...
        self.entry_monto = tk.Entry(
            self.toplevel_tarjeta, textvariable=self.monto_tarjeta, width=15, justify="right", font=("Arial", 11))
        ...
        self.toplevel_tarjeta.bind(
            "<Return>", lambda x: self.validar_monto_tarjeta())
        self.toplevel_tarjeta.bind(
            "<Escape>", lambda x: self.toplevel_tarjeta.destroy())
    ...
    def toplevel_registros(self):  # --- configura ventana para entrada de datos
        ...
        self.lbl_fecha_inicial = tk.Label(
            self.toplevel_reg, text="DESDE", relief="ridge", width=9, bg="lightblue", fg="black", font=("Arial", 10))
        self.lbl_fecha_inicial.place(x=12, y=10)
        self.entry_fecha_inicial = tk.Entry(
            self.toplevel_reg, textvariable=self.fecha_inicial, width=10, font=("Arial", 11))
        self.entry_fecha_inicial.place(x=92, y=10)
        self.lbl_fecha_final = tk.Label(
            self.toplevel_reg, text="HASTA", width=9, relief="ridge", bg="lightblue", fg="black", font=("Arial", 10))
        self.lbl_fecha_final.place(x=200, y=10)
        self.entry_fecha_final = tk.Entry(
            self.toplevel_reg, textvariable=self.fecha_final, width=10, font=("Arial", 11))
        self.entry_fecha_final.place(x=280, y=10)
        self.lbl_relleno = tk.Label(
            self.toplevel_reg, text="", width=45, relief="ridge", bg=None, fg="black", font=("Arial", 10))
        self.lbl_relleno.place(x=398, y=10)
        self.btn_estado_vta = tk.Button(
            self.toplevel_reg, text="INCLUIR / ANULAR", width=20, command=self.cambiar_estado_venta)
        self.btn_estado_vta.place(x=798, y=8)
        self.btn_resumen_vta = tk.Button(
            self.toplevel_reg, text=" VER RESUMEN VENTAS ", width=20, command=self.toplevel_resumen_ventas)
        self.btn_resumen_vta.place(x=984, y=8)

        self.separator_1h = ttk.Separator(
            self.toplevel_reg, orient="horizontal")
        self.separator_1h.place(x=0, y=39, relwidth=1, anchor="w")

        self.lbl_title_ventas = tk.Label(
            self.toplevel_reg, text="Ventas", font=("Arial", 10)).place(x=12, y=45)

        self.separator_2h = ttk.Separator(
            self.toplevel_reg, orient="horizontal")
        self.separator_2h.place(x=0, y=302, relwidth=1, anchor="w")

        self.lbl_title_detalle = tk.Label(
            self.toplevel_reg, text="Detalle", font=("Arial", 10)).place(x=12, y=308)

        self.entry_fecha_inicial.config(
            state="readonly", disabledforeground="black")
        self.entry_fecha_final.config(
            state="readonly", disabledforeground="black")

        self.entry_fecha_inicial.bind(
            "<Button-1>", lambda x: self.set_modo_fecha_inicial())
        self.entry_fecha_final.bind(
            "<Button-1>", lambda x: self.set_modo_fecha_final())
        # destruye ventana al apretar Escape
        self.toplevel_reg.bind(
            "<Escape>", lambda x: self.toplevel_reg.destroy())
    ...
    def toplevel_calendario(self):
        ...
        self.tl_calendario.bind(
            "<Escape>", lambda x: self.tl_calendario.destroy())
    ...
    def validar_fechas(self):
        ...
        if ts_final < ts_inicio:
            ...
            messagebox.showinfo(
                message="POR FAVOR VERIFIQUE LAS FECHAS !!!", title="INFO", parent=self.toplevel_reg)
        else:
    ...
    def treeview_archivo(self, filas=10):
        columnas = ("FECHA", "HORA", "NUMERACION",
                    "EFECTIVO", "TARJETA", "TOTAL", "ESTADO")
        self.tabla_archivo = ttk.Treeview(
            self.toplevel_reg, height=filas, show="headings", selectmode="browse", columns=(columnas))
        # selectmode="browse" no permite que se seleccione mas de una fila
        self.tabla_archivo.place(x=12, y=70)

        self.scroll_archivo = tk.Scrollbar(
            self.toplevel_reg, orient="vertical", command=self.tabla_archivo.yview)
        ...
        self.tabla_archivo.column(
            "FECHA", anchor=tk.CENTER, width=160, minwidth=160)
        self.tabla_archivo.column(
            "HORA", anchor=tk.CENTER, width=160, minwidth=160)
        self.tabla_archivo.column(
            "NUMERACION", anchor=tk.CENTER, width=160, minwidth=160)
        self.tabla_archivo.column(
            "EFECTIVO", anchor=tk.E, width=160, minwidth=160)
        self.tabla_archivo.column(
            "TARJETA", anchor=tk.E, width=160, minwidth=160)
        self.tabla_archivo.column(
            "TOTAL", anchor=tk.E, width=160, minwidth=160)
        self.tabla_archivo.column(
            "ESTADO", anchor=tk.CENTER, width=160, minwidth=160)

        # --- indicar cabecera
        # self.tabla_archivo.heading("#0", text="", anchor=tk.CENTER)
        self.tabla_archivo.heading(
            "#1", text="FECHA", anchor=tk.CENTER)
        self.tabla_archivo.heading(
            "#2", text="HORA", anchor=tk.CENTER)
        self.tabla_archivo.heading(
            "#3", text="NUMERACION", anchor=tk.CENTER)
        self.tabla_archivo.heading(
            "#4", text="EFECTIVO", anchor=tk.CENTER)
        self.tabla_archivo.heading(
            "#5", text="TARJETA", anchor=tk.CENTER)
        self.tabla_archivo.heading(
            "#6", text="TOTAL", anchor=tk.CENTER)
        self.tabla_archivo.heading(
            "#7", text="ESTADO", anchor=tk.CENTER)

        self.tabla_archivo.bind("<<TreeviewSelect>>",
                                lambda x: self.seleccionar_item_archivo())
    ...
    def cargar_treeview_archivo(self):
        ...
        for item in self.ventas_selecc:
            ...
            lista = (fecha, hora, numeracion,
                     f"{efectivo:.2f}", f"{tarjeta:.2f}", f"{total:.2f}", estado)
            self.tabla_archivo.insert("", tk.END, text="", values=(lista))

    def treeview_archivo_detalle(self, filas=6):
        columnas = ("CODIGO", "ARTICULO", "CANT.", "PRECIO",
                    "SUBTOTAL", "DTO. %", "DTO. $", "TOTAL", "DISTRIB.")
        self.tabla_archivo_detalle = ttk.Treeview(
            self.toplevel_reg, show="headings", height=filas, columns=(columnas))
        self.tabla_archivo_detalle.place(x=12, y=332)

        self.scroll_archivo_detalle = tk.Scrollbar(
            self.toplevel_reg, orient="vertical", command=self.tabla_archivo_detalle.yview)
        self.scroll_archivo_detalle.place(x=1140, y=355, height=125)
        self.tabla_archivo_detalle.configure(
            yscrollcommand=self.scroll_archivo_detalle.set)

        # self.tabla_archivo_detalle.column("#0", width=0, stretch=tk.NO , minwidth=100)
        self.tabla_archivo_detalle.column(
            "CODIGO", anchor=tk.W, width=95, minwidth=95)
        self.tabla_archivo_detalle.column(
            "ARTICULO", anchor=tk.W, width=415, minwidth=415)
        self.tabla_archivo_detalle.column(
            "CANT.", anchor=tk.CENTER, width=50, minwidth=50)
        self.tabla_archivo_detalle.column(
            "PRECIO", anchor=tk.E, width=95, minwidth=95)
        self.tabla_archivo_detalle.column(
            "SUBTOTAL", anchor=tk.E, width=95, minwidth=95)
        self.tabla_archivo_detalle.column(
            "DTO. %", anchor=tk.E, width=70, minwidth=70)
        self.tabla_archivo_detalle.column(
            "DTO. $", anchor=tk.E, width=70, minwidth=70)
        self.tabla_archivo_detalle.column(
            "TOTAL", anchor=tk.E, width=95, minwidth=95)
        self.tabla_archivo_detalle.column(
            "DISTRIB.", anchor=tk.CENTER, width=135, minwidth=135)

        # --- indicar cabecera
        # self.tabla_archivo_detalle.heading("#0", text="", anchor=tk.CENTER)
        self.tabla_archivo_detalle.heading(
            "#1", text="CODIGO", anchor=tk.CENTER)
        self.tabla_archivo_detalle.heading(
            "#2", text="ARTICULO", anchor=tk.CENTER)
        self.tabla_archivo_detalle.heading(
            "#3", text="CANT.", anchor=tk.CENTER)
        self.tabla_archivo_detalle.heading(
            "#4", text="PRECIO", anchor=tk.CENTER)
        self.tabla_archivo_detalle.heading(
            "#5", text="SUBTOTAL", anchor=tk.CENTER)
        self.tabla_archivo_detalle.heading(
            "#6", text="DTO. %", anchor=tk.CENTER)
        self.tabla_archivo_detalle.heading(
            "#7", text="DTO. $", anchor=tk.CENTER)
        self.tabla_archivo_detalle.heading(
            "#8", text="TOTAL", anchor=tk.CENTER)
        self.tabla_archivo_detalle.heading(
            "#9", text="DISTRIB.", anchor=tk.CENTER)
    ...
    def cargar_treeview_archivo_detalle(self, indice):
        ...
        self.tabla_archivo_detalle.delete(
            *self.tabla_archivo_detalle.get_children())
        ...
        for item in self.ventas_selecc[indice]["articulos"]:
            ...
            lista = (codigo, articulo, cantidad, f"{precio:.2f}", f"{subtotal:.2f}",
                     f"{desc_porc:.2f} %", f"{desc_pesos:.2f}", f"{total:.2f}", distribuidora)
            self.tabla_archivo_detalle.insert(
                "", tk.END, text="", values=(lista))
    ...
    def ver_archivos_normalizados(self):
        ...
        archivos = filedialog.askopenfilenames(initialdir=ruta, filetypes=[
                                               ("Archivos Excel", "*.csv")])
        print(archivos)
        if archivos:
            for i in archivos:
    ...
    def toplevel_resumen_ventas(self):
        ...
        self.lbl_fecha_txt = tk.Label(
            self.toplevel_resumen, text=f"Desde  {self.fecha_inicial.get()}  hasta  {self.fecha_final.get()}",
            font=("Arial", 12)).grid(row=0, column=0, columnspan=2)
        self.separador1 = tk.Label(self.toplevel_resumen, text="-"*60,
                                   font=("Arial", 12)).grid(row=1, column=0, columnspan=2)
        self.lbl_bruto_txt = tk.Label(
            self.toplevel_resumen, text="TOTAL BRUTO: ", font=("Arial", 12)).grid(row=2, column=0, sticky="w")
        self.lbl_bruto_val = tk.Label(
            self.toplevel_resumen, text=f"{total_bruto:.2f}", font=("Arial", 12)).grid(row=2, column=1, sticky="e")
        self.lbl_desc_txt = tk.Label(
            self.toplevel_resumen, text="TOTAL DESCUENTOS: ", font=("Arial", 12)).grid(row=3, column=0, sticky="w")
        self.lbl_desc_val = tk.Label(
            self.toplevel_resumen, text=f"{total_descuentos:.2f}", font=("Arial", 12)).grid(row=3, column=1, sticky="e")
        self.lbl_ventas_txt = tk.Label(
            self.toplevel_resumen, text="TOTAL VENTAS: ", font=("Arial", 12)).grid(row=4, column=0, sticky="w")
        self.lbl_ventas_val = tk.Label(
            self.toplevel_resumen, text=f"{total_ventas:.2f}", font=("Arial", 12)).grid(row=4, column=1, sticky="e")
        self.lbl_efectivo_txt = tk.Label(
            self.toplevel_resumen, text="TOTAL EFECTIVO: ", font=("Arial", 12)).grid(row=5, column=0, sticky="w")
        self.lbl_efectivo_val = tk.Label(
            self.toplevel_resumen, text=f"{total_efectivo:.2f}", font=("Arial", 12)).grid(row=5, column=1, sticky="e")
        self.lbl_tarjeta_txt = tk.Label(
            self.toplevel_resumen, text="TOTAL TARJETA: ", font=("Arial", 12)).grid(row=6, column=0, sticky="w")
        self.lbl_tarjeta_val = tk.Label(
            self.toplevel_resumen, text=f"{total_tarjeta:.2f}", font=("Arial", 12)).grid(row=6, column=1, sticky="e")
        self.separador2 = tk.Label(
            self.toplevel_resumen, text="-"*60, font=("Arial", 12)).grid(row=7, column=0, columnspan=2)
        self.lbl_cant_ventas = tk.Label(
            self.toplevel_resumen, text="CANTIDAD VENTAS:", font=("Arial", 12)).grid(row=8, column=0, sticky="e")
        self.lbl_tarjeta_val = tk.Label(
            self.toplevel_resumen, text=f"{cantidad_ventas}", font=("Arial", 12)).grid(row=8, column=1, sticky="W")

        self.toplevel_resumen.bind(
            "<Return>", lambda x: self.toplevel_resumen.destroy())
        self.toplevel_resumen.bind(
            "<Escape>", lambda x: self.toplevel_resumen.destroy())
        ...
        try:
            ...
            if seleccion != ():
                if messagebox.askyesno(title="CAMBIAR ESTADO",
                    ...
                    venta_select = self.tabla_archivo.item(
                        seleccion)["values"][2]  # numero de venta
        ...
        if self.modelo_distr_norm.get() == "":
            ...
            if lista_archivos == []:
                ...
                for archivo in lista_archivos:
                    nuevo_archivo = normalizador.normalizar(
                        archivo, distribuidora)
                    nuevos_archivos.append(nuevo_archivo)

                messagebox.showinfo(title="INFO", message=m1 if len(
                    lista_archivos) == 1 else m2, parent=self.ventana_normalizar)
                #    self.ver_archivos_normalizados()

                if messagebox.askyesno(title="ELIMINAR REGISTROS", message=m3, parent=self.ventana_normalizar):
                    self.eliminar_archivos_anteriores(
                        nuevos_archivos, distribuidora)
    ...
    def get_fechas_seleccionadas(self):
        ts_inicio = datetime.timestamp(datetime.strptime(
            self.fecha_inicial.get(), "%d-%m-%Y"))
        ts_final = datetime.timestamp(datetime.strptime(self.fecha_final.get(
        ), "%d-%m-%Y"))+86399  # le agrego 23hs 59min 59seg (1 dia = 86400seg)
...
class Ventas():
    ...
    def grabar_venta(self):
        ...
        for item in self.items_vta:
            values = [str(item.codigo_vta), item.articulo_vta, int(item.cantidad_vta),
                      float(item.descuento_vta), float(item.precio_vta), item.distribuidora_vta]
